...
class MedstsNet(nn.Module):
	def __init__(self, bert, graph_encoder, classifier, classifier_c, classifier_type, config):
	...
	def forward(self, input_ids, attention_mask, token_type_ids, data_a, data_b, mode='medsts'):
		...
		# print('shape of xlnet output: {}'.format(pooled_output.shape))
		pooled_output = self.sequencesummary(pooled_output)
		# print('shape of sequecesummary output: {}'.format(pooled_output.shape))

		graph_output = self.graph_encoder(data_a, data_b)
		# print('shape of graph output: {}'.format(graph_output.shape))
