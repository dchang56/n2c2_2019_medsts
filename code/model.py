...
class GCNNet(nn.Module):
	...
	def forward(self, data, do_layernorm):
		x, edge_index, batch = data.x, data.edge_index, data.batch
		x = self.activation(self.conv1(x, edge_index))
		# x = F.dropout(x, p=0.5, training=self.training)
		if do_layernorm:
			x2 = self.activation(self.conv2(x, edge_index))
			# x2 = F.dropout(x, p=0.5, training=self.training)
...
class GraphEncoder(nn.Module):
	...
	def forward(self, data_a, data_b, do_layernorm):
		x1 = self.gcnnet(data_a, do_layernorm)
		x2 = self.gcnnet(data_b, do_layernorm)
...
class MedstsNet(nn.Module):
	def __init__(self, bert, graph_encoder, classifier, classifier_c, classifier_c2, classifier_type):
		...
		self.classifier_c2 = classifier_c2
		self.classifier_type = classifier_type
	def forward(self, input_ids, attention_mask, token_type_ids, data_a, data_b, mode, do_layernorm):
		outputs = self.text_encoder(input_ids, token_type_ids, attention_mask)
		pooled_output = outputs[1]

		graph_output = self.graph_encoder(data_a, data_b, do_layernorm)
		...
		if mode=='medsts':
			...
			logits = self.classifier_c2(out)
		elif mode=='medsts_type':
			logits = self.classifier_type(out)

		return logits

class MedstsNet_Textonly(nn.Module):
	def __init__(self, bert, classifier, classifier_c, classifier_c2, classifier_type):
		super(MedstsNet_Textonly, self).__init__()
		self.text_encoder = bert
		self.dropout = nn.Dropout(p=0.5)
		self.classifier = classifier
		self.classifier_c = classifier_c
